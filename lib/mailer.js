...
export async function sendPasswordChangedEmail(to, name) {
  ...
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || `"Movie Finder" <${process.env.EMAIL_USER}>`,
    to,
    subject: "Your Movie Finder password was updated",
    html: `
      <div style="font-family:sans-serif;background:#0a0a0a;color:#fff;padding:40px;max-width:500px;margin:auto;border-radius:16px;">
        <h1 style="color:#e50914;">Movie Finder</h1>
        <h2 style="margin-top:12px;">Password updated successfully</h2>
        <p style="color:#aaa;line-height:1.7;">
          Hi ${name || "there"}, your password was changed successfully. If you made this update, no further action is needed.
        </p>
        <div style="margin-top:24px;padding:16px;background:#1a1a1a;border-radius:12px;border:1px solid #333;">
          <p style="margin:0;color:#aaa;font-size:13px;">
            If you did not change your password, reset it immediately and contact support at moviefinderforyou@gmail.com.
          </p>
        </div>
      </div>
    `,
...
export async function sendLoginThankYouEmail(to, name) {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) return;
  ...
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || `"Movie Finder" <${process.env.EMAIL_USER}>`,
    to,
    subject: "Thanks for signing in to Movie Finder",
    html: `
      <div style="font-family:sans-serif;background:#0a0a0a;color:#fff;padding:40px;max-width:500px;margin:auto;border-radius:16px;">
        <h1 style="color:#e50914;">Movie Finder</h1>
        <h2 style="margin-top:12px;">Thanks for signing in, ${name || "there"}</h2>
        <p style="color:#aaa;line-height:1.7;">
          Your account is active and your latest preferences are ready to shape recommendations, theater hints, and streaming availability.
        </p>
        <p style="color:#aaa;line-height:1.7;">
          If this sign-in was not you, reset your password and contact moviefinderforyou@gmail.com.
        </p>
      </div>
    `,
  });
