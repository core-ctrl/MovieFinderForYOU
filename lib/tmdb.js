...
export async function fetchWatchProviders(id, type = "movie") {
  const d = await req(`${TMDB_BASE}/${type}/${id}/watch/providers`, p());
  return d.results?.IN || d.results?.US || null;
}
...
function pickSafeTrailer(videos) {
  ...
  const safe = (v) => v.site === "YouTube" && !banned.some((w) => v.name.toLowerCase().includes(w));
  for (const type of ["Official Trailer", "Trailer", "Teaser", "Clip", "Featurette"]) {
    const match = videos.filter((v) => v.site === "YouTube" && v.type.toLowerCase() === type.toLowerCase()).filter(safe);
